...
def test_sisso_regressor(mocker):
    with ScratchDir('.'):
        def copy_sisso_out():
            shutil.copy(os.path.join(TEST_FILES_DIR, 'runs', 'perfect_linear_5pts', 'SISSO.out'), 'SISSO.out')
        mocker.patch.object(pysisso.sklearn.Custodian, 'run', return_value=[], side_effect=copy_sisso_out)
